...
Instead of matching keywords, Qdrant retrieves documents based on meaning.
...
print(urllib.request.urlopen("http://qdrant:8080/v1/.well-known/ready").read().decode())
PY
```
...
# Step 3.2 — Update the Database Schema (`qdrant_client.py`)

Open:

```
ingestion-api/app/qdrant_client.py
```
...
```bash
nano ingestion-api/app/qdrant_client.py
```
