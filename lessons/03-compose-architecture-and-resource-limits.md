...
In `docker-compose.yml`, find the `qdrant:` service.

Key things to notice:

- `mem_limit: 8g` (resource boundary)
- it is on `networks: internal`
- it uses the `qdrant_data` volume
...
- Qdrant calls it using `TRANSFORMERS_INFERENCE_API=http://text2vec-transformers:8080`
- it is internal-only
...
- it depends on Qdrant, transformer module, and Ollama
...
- The API can call Qdrant at `http://qdrant:8080`
- Qdrant can call transformers at `http://text2vec-transformers:8080`
...
- `qdrant_data` — your stored objects + vectors
...
## Step 5 — Memory limits (Qdrant + Ollama capped at 8 GB)
...
- You can point to where the 8 GB limits are set for Qdrant and Ollama.
- You can explain what `docker stats` shows in one sentence.

[Lesson 4](https://github.com/JacksonHolmes01/Training-Module-Production-Grade-RAG-Infrastructure-Qdrant/blob/main/lessons/04-edge-auth-with-nginx.md)
