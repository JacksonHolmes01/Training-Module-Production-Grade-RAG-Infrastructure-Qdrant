...
1. API calls Qdrant GraphQL with `nearText`
...
- Fix ingestion / vectorizer / Qdrant first

Go back to:

- **Lesson 3 — Docker & Service Health**  
  (verify Qdrant and text2vec-transformers readiness)
...
2. Calls `retrieve_sources()`  
   - Queries Qdrant using semantic search  
...
3. A GraphQL `nearText` query is sent to Qdrant.
4. Qdrant performs semantic search using stored embeddings.
...
- Confirm objects exist:
  ...
  docker exec -i ingestion-api python -c "import urllib.request; print(urllib.request.urlopen('http://qdrant:8080/v1/objects?class=LabDoc').read()[:300])"
...
3. Qdrant performs semantic retrieval  
...
- `retrieve_sources()` queries Qdrant via GraphQL  
...
- **Retrieval empty** → ingestion/vectorizer/Qdrant problem  
...
[Lesson 8](https://github.com/JacksonHolmes01/Training-Module-Production-Grade-RAG-Infrastructure-Qdrant/blob/main/lessons/08-gradio-chat-ui.md)
