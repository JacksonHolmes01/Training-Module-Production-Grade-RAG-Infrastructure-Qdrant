...
- **text2vec-transformers**  
  Generates embeddings (vectors) so Qdrant can do semantic search.
- **Ollama**  
...
- **Ingestion API (FastAPI)**  
  ...
  - retrieves top relevant docs from Qdrant during chat
...
- Qdrant is internal-only (`qdrant:8080`)
...
2. The API retrieves relevant documents from Qdrant.
...
- **Service**: a container in docker-compose.yml (e.g., `qdrant`, `nginx`).
...
- You can explain why Qdrant is not exposed to the host.
