...
The container name "/qdrant" is already in use...
```

It means Docker already has a container named `qdrant` on your machine.
...
Both Lab 1 and Lab 2 use the container name `qdrant`, so only one can run at a time.
