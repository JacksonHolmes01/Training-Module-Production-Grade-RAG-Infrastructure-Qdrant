"""
Host-mode ingestion helper.

Run from repo root:
    python security-memory/scripts/ingest_security_memory.py

Assumes:
- Qdrant reachable at http://localhost:6333
- Embeddings reachable at EMBEDDINGS_BASE_URL (default: http://localhost:8089)

If you do NOT expose embeddings to host, use container-mode:
    docker exec -i ingestion-api python -m app.security_memory.ingest
"""
import os, re, uuid, asyncio
from pathlib import Path
from typing import List, Dict, Any
import httpx

QDRANT_URL = os.getenv("QDRANT_URL", "http://localhost:6333").rstrip("/")
EMBEDDINGS_BASE_URL = os.getenv("EMBEDDINGS_BASE_URL", "http://localhost:8089").rstrip("/")

SECURITY_COLLECTION = os.getenv("SECURITY_COLLECTION", "ExpandedVSCodeMemory")
SECURITY_CHUNK_CHARS = int(os.getenv("SECURITY_CHUNK_CHARS", "1200"))
SECURITY_CHUNK_OVERLAP = int(os.getenv("SECURITY_CHUNK_OVERLAP", "200"))
EMBEDDINGS_DIM = int(os.getenv("EMBEDDINGS_DIM", "384"))

DATA_DIR = Path(os.getenv("SECURITY_DATA_DIR", "security-memory/data"))

TAG_KEYS = ["nist","cis","mitre","owasp","docker","kubernetes","linux","cloud","iam","sdlc","appsec","containers"]

def _normalize(text: str) -> str:
    text = text.replace("\r\n", "\n")
    text = re.sub(r"\n{3,}", "\n\n", text).strip()
    return text

def _chunk(text: str) -> List[str]:
    text = _normalize(text)
    if not text:
        return []
    out = []
    start = 0
    n = len(text)
    while start < n:
        end = min(n, start + SECURITY_CHUNK_CHARS)
        piece = text[start:end].strip()
        if piece:
            out.append(piece)
        if end == n:
            break
        start = max(0, end - SECURITY_CHUNK_OVERLAP)
    return out

def _guess_tags(path: Path) -> List[str]:
    s = str(path.as_posix()).lower()
    tags = [k for k in TAG_KEYS if k in s]
    return sorted(set(tags))

async def _ensure_collection():
    async with httpx.AsyncClient(timeout=15.0) as client:
        r = await client.get(f"{QDRANT_URL}/collections/{SECURITY_COLLECTION}")
        if r.status_code == 200:
            return
        payload = {"vectors": {"size": EMBEDDINGS_DIM, "distance": "Cosine"}}
        cr = await client.put(f"{QDRANT_URL}/collections/{SECURITY_COLLECTION}", json=payload)
        cr.raise_for_status()

async def _embed(texts: List[str]) -> List[List[float]]:
    async with httpx.AsyncClient(timeout=180.0) as client:
        r = await client.post(f"{EMBEDDINGS_BASE_URL}/embed", json={"inputs": texts})
        r.raise_for_status()
        data = r.json()
        if not isinstance(data, list):
            raise ValueError("Unexpected embeddings response shape")
        return data

async def _upsert(points: List[Dict[str, Any]]):
    async with httpx.AsyncClient(timeout=90.0) as client:
        r = await client.put(f"{QDRANT_URL}/collections/{SECURITY_COLLECTION}/points?wait=true", json={"points": points})
        r.raise_for_status()

async def main():
    if not DATA_DIR.exists():
        raise SystemExit(f"Missing {DATA_DIR}. Put docs there first.")
    files = [p for p in DATA_DIR.rglob("*") if p.is_file() and p.suffix.lower() in {".md", ".txt"}]
    if not files:
        raise SystemExit(f"No .md/.txt found under {DATA_DIR}.")

    await _ensure_collection()

    BATCH = 32
    texts, metas = [], []
    total = 0

    for fp in files:
        raw = fp.read_text(encoding="utf-8", errors="ignore")
        chunks = _chunk(raw)
        if not chunks:
            continue

        title = fp.stem.replace("_", " ").replace("-", " ").strip()
        source = fp.parent.name
        tags = _guess_tags(fp)

        for i, ch in enumerate(chunks):
            texts.append(ch)
            metas.append({"title": title, "source": source, "tags": tags, "chunk_index": i, "doc_path": str(fp.as_posix()), "text": ch})
            total += 1
            if len(texts) >= BATCH:
                vecs = await _embed(texts)
                pts = [{"id": str(uuid.uuid4()), "vector": v, "payload": m} for m, v in zip(metas, vecs)]
                await _upsert(pts)
                texts, metas = [], []

    if texts:
        vecs = await _embed(texts)
        pts = [{"id": str(uuid.uuid4()), "vector": v, "payload": m} for m, v in zip(metas, vecs)]
        await _upsert(pts)

    print(f"[host-ingest] complete: files={len(files)} chunks={total} collection={SECURITY_COLLECTION}")

if __name__ == "__main__":
    asyncio.run(main())
