...
Open `docker-compose.yml` in your IDE by clicking it in the file explorer. Then open the AI chat panel (`Ctrl+Alt+I` in VS Code, `Cmd+L` in Cursor) and paste the following, replacing the placeholders with the text you copied in Step 3 and the contents of your `docker-compose.yml`:
...
Focus especially on: exposed ports, plaintext secrets, privileged mode, host volume mounts, network exposure, and missing resource limits.
