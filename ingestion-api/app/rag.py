...
def _headers() -> dict:
    """
    If Qdrant auth is enabled in your deployment, set QDRANT_API_KEY.
...
async def ensure_schema() -> None:
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        # Qdrant accepts POST /v1/schema with {"class": "..."}
...
async def retrieve_sources(query: str, k: Optional[int] = None) -> List[Dict[str, Any]]:
    """
    Retrieve top-k documents from Qdrant using nearText on LabDoc.
    """
    await ensure_schema()
    ...
    async with httpx.AsyncClient(timeout=25.0) as client:
        r = await client.post(f"{QDRANT_BASE}/v1/graphql", json=gql, headers=_headers())
        r.raise_for_status()
